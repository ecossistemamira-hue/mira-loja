...
// Sem paths no build: cada PDP é gerada sob demanda no 1º acesso e fica em
// cache ISR (revalidate acima) — sem isto o Next trata a rota como dinâmica.
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Canonical segue a URL em espanhol (sem prefixo); /pt entra em hreflang.
  const canonical = `${SITE_URL}/p/${produto.slug ?? produto.id}`
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        es: canonical,
        'pt-BR': `${SITE_URL}/pt/p/${produto.slug ?? produto.id}`,
      },
    },
    openGraph: {
      title: produto.nome,
